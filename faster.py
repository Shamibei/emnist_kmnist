...
def train(X_train, y_train, input_size, hidden_size, output_size, epochs, batch_size, lr, decay_rate, momentum, patience, reg_lambda):
    ...
    gradients = {key: np.zeros_like(value) for key, value in weights.items()}
...
if __name__ == "__main__":
    ...
    trained_weights, loss_history = train(train_images, train_labels_encoded, input_size, hidden_size, output_size,
                                          epochs, batch_size, lr, decay_rate, momentum, patience, reg_lambda)
    ...
    plt.close()  # Close the plot to avoid memory buildup
    ...
    # Visualize predictions and save them as images
    for i in range(5):
        plt.imshow(test_images[i].reshape(28, 28), cmap='gray')
        plt.title(f"Predicted: {predictions[i]}, Actual: {test_labels[i]}")
        plt.savefig(f'prediction_{i}.png')  # Save the plot to a file
        plt.close()  # Close the plot to avoid memory buildup

    # Display all the plots at once (optional)
    plt.show()  # This will display the plots if needed
